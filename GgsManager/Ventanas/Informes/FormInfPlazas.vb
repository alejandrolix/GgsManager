    ...
    Private Sub PlLibreRb_CheckedChanged(sender As Object, e As EventArgs) Handles PlLibreRb.CheckedChanged
        ...
        If PlLibreRb.Checked Then
            ...
            Me.Adaptador = New MySqlDataAdapter(String.Format("SELECT Plz.IdPlaza, CONCAT(Cli.Nombre, ' ', Cli.Apellidos) AS 'Cliente', Veh.Matricula, Veh.Marca, Veh.Modelo
                                                               ...
                                                               WHERE  Plz.IdGaraje = {0} AND Plz.IdSituacion = 1;", IdGarajeSelec), conexion)
        ...
        If PlOcupadaRb.Checked Then
            ...
            Me.Adaptador = New MySqlDataAdapter(String.Format("SELECT Plz.IdPlaza, CONCAT(Cli.Nombre, ' ', Cli.Apellidos) AS 'Cliente', Veh.Matricula, Veh.Marca, Veh.Modelo
                                                               FROM   Plazas Plz
	                                                                  JOIN Vehiculos Veh ON Veh.IdPlaza = Plz.IdPlaza
                                                                      JOIN Clientes Cli ON Cli.IdCliente = Veh.IdCliente       
                                                               WHERE  Plz.IdGaraje = {0} AND Plz.IdSituacion = 2;", IdGarajeSelec), conexion)
            Adaptador.Fill(dtPlazas, "Plazas")
            conexion.Close()
        ...
        If PlTodasRb.Checked Then
            ...
            Me.Adaptador = New MySqlDataAdapter(String.Format("SELECT Plz.IdPlaza, CONCAT(Cli.Nombre, ' ', Cli.Apellidos) AS 'Cliente', Veh.Matricula, Veh.Marca, Veh.Modelo
                                                               FROM   Plazas Plz
	                                                                  JOIN Vehiculos Veh ON Veh.IdPlaza = Plz.IdPlaza
                                                                      JOIN Clientes Cli ON Cli.IdCliente = Veh.IdCliente                                                                      
                                                               WHERE  Plz.IdGaraje = {0} AND Plz.IdSituacion BETWEEN 1 AND 2;", IdGarajeSelec), conexion)
            Adaptador.Fill(dtPlazas, "Plazas")
            conexion.Close()
